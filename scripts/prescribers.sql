...
SELECT pn.npi, pr.nppes_provider_first_name, pr.nppes_provider_last_org_name, pr.specialty_description, SUM(pn.total_claim_count)
...
SELECT pr.specialty_description, SUM(pn.total_claim_count)
FROM prescription AS pn
INNER JOIN prescriber AS pr
USING (npi)
...
SELECT pr.specialty_description, 
	(SELECT SUM(pn.total_claim_count)
	FROM prescription AS pn
	INNER JOIN drug AS dg
	USING (drug_name)
	WHERE dg.opioid_drug_flag = 'Y')/SUM(pn.total_claim_count) AS sum_op
FROM prescriber AS pr
...
-- sum(case statement, opiod claims, opiod total, opioid perc)
